...
def prep_data(data_dir):
    ...
    for crt_dir in data_dir:
        ...
        numericFacts = pd.read_csv(
            f'{crt_dir}/num.txt', 
            sep='\t', 
            low_memory=False
        )
        # Join numeric facts with submission data on 'adsh' (accession number)
        joined = numericFacts.join(
            submissions.set_index('adsh'), 
            on='adsh'
        )
...
def tags_w_percent_str(df_joined):
    ...
    top_tags = pd.read_csv('tag_stats_sorted.csv')
    top_tags['rank'] = range(len(top_tags))
    percent_tags = pd.merge(top_tags, percent_tags, on='tag')
